...
const steps: Step[] = [
  ...
  },
  {
    ...
    tagline: 'Cloudflare Whisper Turbo first, Groq backup on limit',
    badge: 'Whisper Large v3 Turbo + Groq Backup',
    icon: Music,
    description:
      'The audio is transcribed with millisecond word timestamps, Cloudflare Workers AI Whisper Large v3 Turbo first — always. If Cloudflare hits its daily limit, Groq Whisper takes over automatically as backup. Speech fillers are indexed for optional trimming to keep clips tight.',
    details: [
      'Sub-word timestamp synchronization for flawless animated caption alignment',
      'Strict order: Cloudflare Turbo primary, Groq fallback only on limit or error — Groq is never tried first',
      'Automatic flagging of English + Hindi/Hinglish fillers ("um", "मतलब", "यार") for optional trimming',
    ],
    techPill: 'Cloudflare Whisper Turbo • Groq Backup • Word Diarization',
  },
  {
    id: 'score',
    number: '03',
    title: 'Semantic Windowing & Gemini 4D Scoring',
    tagline: 'Context-complete clips, audio-signal boosted, Groq-backed scoring',
    badge: 'Gemini + Groq LLM + Audio Signals',
    icon: Cpu,
    description:
      'Candidate windows (15–90 seconds) snap to sentence boundaries and extend past 35 seconds whenever the thought needs more context — no mid-sentence cuts. Gemini scores each candidate across 4 transparent dimensions with a Groq LLM backup on quota exhaustion, then zero-cost audio signals (energy peaks, speech excitement, clean entry/exit) boost the ranking.',
    details: [
      ...
      'Audio-signal fusion (+8 max): laughter/applause peaks, excitement bursts, resolved exits',
      'Fixed yield: top 10 clips per video, regardless of duration',
    ],
    techPill: 'Gemini API • Groq Backup • Signal Fusion • Semantic Snapping',
  },
  {
    id: 'reframe',
    number: '04',
    ...
    details: [
      ...
      'Clips run 15–90s and always end on a resolved thought — never cut mid-sentence',
