...
export default function HeroUploader() {
  ...
  return (
    <div id="app" className="w-full max-w-5xl mx-auto">
      {/* Main Tool Container */}
      <div className="rounded-2xl border border-[#242938] bg-[#141620] shadow-2xl p-4 sm:p-6 lg:p-8">
        {/* Top Controls Bar */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[#242938] pb-5">
          {/* Input Method Switcher */}
          <div className="flex items-center gap-1 rounded-xl bg-[#0A0B10] p-1 border border-[#242938]">
            <button
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <label className="text-xs font-medium text-[#9AA2B6]">Audio:</label>
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as any)}
                className="rounded-lg border border-[#2B3040] bg-[#0A0B10] px-2.5 py-1.5 text-xs font-medium text-white focus:outline-none focus:border-[#FF5722]"
              >
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs font-medium text-[#9AA2B6]">Captions:</label>
              <select
        ...
        <div className="mt-6">
          {activeTab === 'upload' ? (
            <div className="relative rounded-xl border-2 border-dashed border-[#2B3040] bg-[#0A0B10]/60 p-8 text-center hover:border-[#FF5722]/60 transition-colors cursor-pointer group">
              <input
                type="file"
              ...
              />
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-[#1E2230] text-[#FF5722] group-hover:scale-105 transition-transform">
              ...
              <p className="mt-3 text-sm font-semibold text-white">
                {selectedFile ? (
                  <span>
                    {selectedFile.name}{' '}
                    <span className="text-xs text-[#9AA2B6]">
              ...
              <p className="mt-1 text-xs text-[#9AA2B6]">
              ...
              {uploadProgress !== null && (
                <div className="mt-4 w-full max-w-md mx-auto">
                  <div className="flex justify-between text-xs text-[#9AA2B6] mb-1">
                    <span>Streaming directly to Cloudflare R2...</span>
                    <span className="font-bold text-[#FF5722]">{uploadProgress}%</span>
                  </div>
                  <div className="w-full bg-[#1E2230] rounded-full h-2 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-[#FF5722] to-[#FFB800] h-2 transition-all duration-150"
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-[#2B3040] bg-[#0A0B10] p-4 flex flex-col sm:flex-row items-center gap-3">
                <input
                  type="url"
                  placeholder="Paste YouTube podcast URL (e.g. https://youtube.com/watch?v=... or https://youtu.be/...)"
                  value={youtubeUrl}
                  onChange={(e) => handleUrlChange(e.target.value)}
                  className="w-full rounded-lg bg-[#141620] border border-[#2B3040] px-3.5 py-2.5 text-sm text-white placeholder-[#626B82] focus:outline-none focus:border-[#FF5722]"
                />
                <button
                  type="button"
                  onClick={handleRunPipeline}
                  disabled={isProcessing}
                  className="w-full sm:w-auto shrink-0 rounded-lg bg-[#FF5722] px-5 py-2.5 text-xs font-bold text-white hover:bg-[#F44336] transition-colors disabled:opacity-50"
                >
              ...
              {youtubeMetadata && (
                <div className="rounded-xl border border-[#2B3040] bg-[#0A0B10] p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  <img
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button
            onClick={handleRunPipeline}
            disabled={isProcessing}
            className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#FF5722] to-[#FF3B30] px-6 py-3 text-sm font-bold text-white shadow-lg hover:opacity-95 transition-opacity disabled:opacity-50"
          >
        ...
        {/* Results Area */}
        {showResults && (
          <div className="mt-10 border-t border-[#242938] pt-8">
            {/* View Mode Toggle: Competitive Wedge Demonstration */}
            <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-xl border border-[#2B3040] bg-[#0A0B10] p-4">
              <div>
              ...
              <div className="flex items-center rounded-lg bg-[#141620] p-1 border border-[#2B3040]">
                <button
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                  {rankedClips.map((clip) => (
                    <div
                      key={clip.id}
                      className="flex flex-col justify-between rounded-xl border border-[#2B3040] bg-[#0A0B10] p-5 hover:border-[#FF5722]/70 transition-all shadow-md"
                    >
                      <div>
                        {/* Header: Rank + Composite Score */}
                        <div className="flex items-center justify-between mb-3">
                          <span className="flex items-center gap-1.5 rounded-md bg-[#FF5722]/15 px-2.5 py-1 text-xs font-black text-[#FF5722]">
                            <Flame className="h-3.5 w-3.5" />
                            RANK #{clip.rank}
                          </span>
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs text-[#9AA2B6]">Score:</span>
                            <span className="text-base font-extrabold text-[#FFB800] font-[var(--font-outfit)]">
                              {clip.score.compositeScore}
                            </span>
                            <span className="text-[10px] text-[#626B82]">/100</span>
                          </div>
                        </div>

                        {/* Timing */}
                        <div className="text-xs text-[#9AA2B6] font-medium mb-2">
                          Timestamp: <strong className="text-white">{clip.startTime}s – {clip.endTime}s</strong> ({clip.duration}s)
                        </div>

                        {/* 4 Dimension Matrix */}
                        <div className="grid grid-cols-2 gap-2 rounded-lg bg-[#141620] p-2.5 text-xs">
                          <div>
                            <span className="text-[#626B82]">Hook:</span>{' '}
                            <span className="font-bold text-white">{clip.score.dimensions.hookStrength}/10</span>
                          </div>
                          <div>
                            <span className="text-[#626B82]">Coherence:</span>{' '}
                            <span className="font-bold text-white">{clip.score.dimensions.standaloneCoherence}/10</span>
                          </div>
                          <div>
                            <span className="text-[#626B82]">Emotion:</span>{' '}
                            <span className="font-bold text-white">{clip.score.dimensions.emotionalPayoff}/10</span>
                          </div>
                          <div>
                            <span className="text-[#626B82]">Trend:</span>{' '}
                            <span className="font-bold text-white">{clip.score.dimensions.topicTrendAlignment}/10</span>
                          </div>
                        </div>

                        {/* Gemini Transparent Reasoning */}
                        <div className="mt-3 text-xs italic text-[#9AA2B6] border-l-2 border-[#FF5722] pl-2.5 py-0.5">
                          "{clip.score.reasoning}"
                        </div>

                        {/* Transcript Snippet */}
                        <p className="mt-3 text-xs text-white leading-relaxed line-clamp-3 bg-[#141620]/70 p-2 rounded">
                      ...
                      <div className="mt-5 pt-3 border-t border-[#242938]">
                        <div className="flex items-center justify-between text-xs text-[#9AA2B6] mb-3">
                          <span>Nudge start/end:</span>
                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => handleNudge(clip.id, 'start', -1)}
                              className="rounded bg-[#1E2230] px-2 py-0.5 hover:bg-[#2B3040] text-white"
                              title="Start -1s"
                            >
                              -1s
                            </button>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-[#FF5722] py-2 text-xs font-bold text-white hover:bg-[#F44336] transition-colors shadow-sm"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center justify-center gap-1 rounded-lg border border-[#2B3040] bg-[#141620] px-2.5 py-2 text-xs font-semibold text-[#FFB800] hover:border-[#FFB800] transition-colors"
                          ...
                          <a
                            ...
                            className="flex items-center justify-center rounded-lg border border-[#2B3040] bg-[#141620] px-2.5 py-2 text-xs text-[#9AA2B6] hover:text-white transition-colors"
            ...
            {viewMode === 'naive' && (
              <div className="rounded-xl border border-[#EF4444]/30 bg-[#0A0B10] p-6">
                <div className="flex items-start gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#9AA2B6] mt-1 leading-relaxed">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {naiveClips.map((chunk) => (
                    <div
                      key={chunk.id}
                      className="rounded-lg border border-[#2B3040] bg-[#141620] p-3 text-xs opacity-75"
                    >
                      <div className="flex items-center justify-between text-[#EF4444] font-bold text-[11px] mb-1">
                        <span>{chunk.id}</span>
                        <span>{chunk.startTime}s – {chunk.endTime}s</span>
                      </div>
                      <p className="text-[#9AA2B6] line-clamp-2 italic">
                        "{chunk.textSnippet}"
                      </p>
